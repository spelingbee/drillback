# Running `restored` from a container

There is an image that bundles `restored`, the Docker CLI, the Compose plugin and
`restic`, for machines where installing four things by hand is the hard part - which
is most NAS boxes.

```
ghcr.io/spelingbee/restored:0.1.0
```
...
sudo mkdir -p /var/lib/restored

docker run --rm \
  --group-add "$DOCKER_GID" \
  -v /var/run/docker.sock:/var/run/docker.sock \
  -v /var/lib/restored:/var/lib/restored \
...
`--workspace /var/lib/restored` and `-v /var/lib/restored:/var/lib/restored` are the
same path on both sides on purpose, and the run will not work otherwise.

`restored` restores your backup into the workspace and then asks the daemon to
...
| mount | why |
|---|---|
| `/var/run/docker.sock` | restored has no daemon of its own. This is the whole mechanism, and the whole risk. |
| `/var/lib/restored` (same path both sides) | the restored data, which the daemon must be able to see. |
...
environment. `restored` never logs restic's environment and never parses it, but it
...
`restored` scrubs it out of the report and the debug log before you see them (ADR-059).
...
There is no configuration of this image, and no setting inside `restored`, that
prevents that; the grant is made by the `-v /var/run/docker.sock` on your command line,
not by anything in here.

That is worth stating plainly, because `restored`'s other security properties are
...
- **You already trust this daemon** with the containers you run on it. `restored` is
  one more.
...
- **The image is not the boundary.** Running `restored` as a binary on the host, which
...
- **Prefer a rootless daemon or a socket proxy** if you have one. `restored` needs
...
0 4 * * 0 docker run --rm --group-add "$(stat -c %g /var/run/docker.sock)" \
  ...
  -v /etc/restic/pass:/run/secrets/restic-pass:ro \
  -e RESTIC_PASSWORD_FILE=/run/secrets/restic-pass \
  -v /var/log/restored:/var/log/restored \
  ghcr.io/spelingbee/restored:0.1.0 \
    check --recipe gitea --source restic --from /mnt/backups/restic \
      --workspace /var/lib/restored \
      --report /var/log/restored/gitea-$(date +\%F).json
```
...
`restored`, `docker-cli`, `docker-cli-compose`, `restic` and `ca-certificates`. Nothing
else.
...
**A full `restored check` from inside the image, against the host daemon.** Docker
...
```text
$ docker run --rm -v /:/host alpine:3.22 mkdir -p /host/var/lib/restored-demo

$ docker run --rm -u 0     -v /var/run/docker.sock:/var/run/docker.sock     -v /var/lib/restored-demo:/var/lib/restored-demo     -v /path/to/restored:/src:ro -v /path/to/release:/rel:ro     -e TMPDIR=/var/lib/restored-demo -e RESTORED_BIN=/rel/restored     -w /src --entrypoint sh restored:dev -c './scripts/demo.sh'
...
destroyed, and restored - by the `restored` binary out of the release archive, running
