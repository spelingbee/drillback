...
- With `MSYS_NO_PATHCONV=1`, every path handed to `docker` **and to `drillback.exe`** has
...
- Scratch lives in `$TMPDIR/drillback-drill/<app>`; `drill_init` removes the previous
...
- Leftover stacks from an interrupted run: `docker compose ls --all`, then
  ...
  `drillback-*`; there is other work on this machine.
